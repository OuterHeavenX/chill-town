...
const World = preload("res://presentation/illustrated_world.gd")
const Hud = preload("res://ui/illustrated_hud.gd")
const Clock = preload("res://core/simulation_clock.gd")
const SAVE_PATH := "user://vale-illustrated-save-v1.json"
var sim: RefCounted
var world: Node2D
var hud: CanvasLayer
...
func _ready() -> void:
	...
	_setup_audio()
	hud.show_message("Bem-vindo ao vale. Construa uma horta: os trabalhadores cuidam de tudo.")
	print("PLAYABLE_READY: Vale dos Vinhedos — Vila ilustrada 0.2")
...
func _process(delta: float) -> void:
	...
	if since_autosave >= 60.0 and not application_paused:
		since_autosave = 0.0
		_write_save("user://vale-illustrated-autosave-v1.json")
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		match event.keycode:
			...
			KEY_F5: _save()
			KEY_F9: _load_save()
			KEY_HOME: world.focus_cell(Vector2i(8,11))
			KEY_M:
				mute = not mute
...
func _write_save(path: String) -> bool:
	var data := {"format":"vale-illustrated-v1","simulation":sim.snapshot(),"clock":clock.capture(),"speed":speed}
...
func _load_save() -> void:
	_load_paths([SAVE_PATH,SAVE_PATH+".bak","user://vale-illustrated-autosave-v1.json"])
...
func _load_paths(paths: Array) -> bool:
	for path in paths:
		...
		file.close()
		if not data is Dictionary or data.get("format") != "vale-illustrated-v1" or not data.get("simulation") is Dictionary or not data.get("clock") is Dictionary:
			continue

extends CanvasLayer
## HUD somente de leitura. Todas as alterações passam por sinais para o root.

signal build_selected(kind: String)
signal army_selected(order: String)
...
const INK := Color("293b35")
const MUTED := Color("687268")
const BRONZE := Color("bb843e")
const WINE := Color("7c405a")
const DANGER := Color("a45038")
const SUCCESS := Color("4d7551")
const BUILD_ORDER := ["house", "farm", "vineyard", "winery", "store", "lumber", "quarry", "training", "barracks"]
...
const SHORT_NAMES := {"house":"Casa", "farm":"Horta", "vineyard":"Parreiral", "winery":"Vinícola", "store":"Armazém", "lumber":"Lenhador", "quarry":"Pedreira", "training":"Treinamento", "barracks":"Quartel"}
const BUILD_HINTS := {"house":"Mais espaço para morar", "farm":"Alimento para crescer", "vineyard":"O começo de cada vinho", "winery":"Uvas viram vinho", "store":"Espaço para os recursos", "lumber":"Madeira para construir", "quarry":"Pedra para a vila", "training":"Novos profissionais", "barracks":"Reforços para a companhia"}
const ORDER_NAMES := {"attack":"Atacar", "defend":"Defender", "regroup":"Reagrupar", "retreat":"Recuar"}
...
class Glyph extends Control:
	...
	func _draw() -> void:
		...
		match kind:
			...
			"lancer", "barracks":
				draw_line(Vector2(28,36),Vector2(28,6),Color("9b744b"),3.0,true)
				draw_colored_polygon(PackedVector2Array([Vector2(28,2),Vector2(24,11),Vector2(32,11)]),Color("9daea8"))
				draw_colored_polygon(PackedVector2Array([Vector2(6,14),Vector2(21,14),Vector2(21,28),Vector2(14,36),Vector2(6,28)]),dark)
				draw_line(Vector2(14,17),Vector2(14,30),gold,2.0,true)
			"archer":
				draw_arc(Vector2(10,20),16,-PI/2,PI/2,24,Color("9b744b"),3.0,true)
				draw_line(Vector2(10,4),Vector2(10,36),dark,1.5,true)
				draw_line(Vector2(5,20),Vector2(35,20),gold,2.0,true)
				draw_colored_polygon(PackedVector2Array([Vector2(38,20),Vector2(31,16),Vector2(31,24)]),dark)
			_:
...
var _army_summary: Label
var _army_status: Label
...
func _make_theme() -> Theme:
	...
	theme.set_stylebox("normal", "Button", _style(Color("e8dec4"), Color("c6b998"), 9))
	theme.set_stylebox("hover", "Button", _style(Color("fff6df"), BRONZE, 9))
	theme.set_stylebox("pressed", "Button", _style(Color("d6c49b"), BRONZE, 9))
	theme.set_stylebox("focus", "Button", _focus_style())
	theme.set_stylebox("disabled", "Button", _style(Color("ddd7c6"), Color("c5c3b6"), 9))
	theme.set_color("font_color", "Button", INK)
	theme.set_color("font_hover_color", "Button", INK)
	theme.set_color("font_pressed_color", "Button", INK)
	theme.set_color("font_disabled_color", "Button", MUTED)
	theme.set_font_size("font_size", "Button", 17)
	theme.set_stylebox("background", "ProgressBar", _style(Color("ddd3b8"), Color.TRANSPARENT, 4, 0))
	theme.set_stylebox("fill", "ProgressBar", _style(SUCCESS, Color.TRANSPARENT, 4, 0))
	return theme

func _style(bg: Color, border: Color = Color("c6b998"), radius: int = 12, padding: int = 12) -> StyleBoxFlat:
	var style := StyleBoxFlat.new()
	style.bg_color = bg
...
func _panel(parent: Node, register_region: bool = true, padding: int = 14) -> PanelContainer:
	...
	var style := _style(PAPER, Color("c8b88e"), 14, padding)
...
func _accent(button: Button, color: Color = INK) -> void:
...
func _make_top_bar() -> void:
	...
	_brand = _label(_brand_box,"Vale dos Vinhedos",21)
	_label(_brand_box,"UMA VILA COM VIDA",11,MUTED)
	for item in ["wood","stone","food","grapes","wine","population"]:
		...
		button.add_theme_stylebox_override("normal",_style(Color("eae1ca"),Color.TRANSPARENT,8,4))
...
func _make_objectives() -> void:
	...
	_objectives_toggle = _button(box,"Objetivos  0/4  −",_toggle_objectives)
	...
	_outcome = _label(_objective_details,"Faça sua vila prosperar",18,WINE,true)
	for i in range(4):
...
func _make_dock() -> void:
	...
	_tabs["build"] = _button(row,"Construir",_toggle_drawer.bind("build"),148)
	_tabs["training"] = _button(row,"Ofícios",_toggle_drawer.bind("training"),122)
	_tabs["army"] = _button(row,"Exército",_toggle_drawer.bind("army"),122)
	_accent(_tabs["build"])
	_spacer(row)
	_village_focus = _button(row,"Vila",_focus_village,70)
	_village_focus.tooltip_text = "Voltar ao centro da vila"
	_target_focus = _button(row,"Objetivo",_focus_target,90)
	_target_focus.tooltip_text = "Ver o acampamento do outro lado do rio"
...
func _make_drawer() -> void:
	...
	_drawer_title = _label(titles,"",24)
...
func _populate_build() -> void:
	...
	for kind in BUILD_ORDER:
		...
		button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		button.custom_minimum_size.y = 101
		...
		var line := _hbox(card,6)
		_glyph(line,kind,30)
		_label(line,SHORT_NAMES[kind],17,WINE if kind == "winery" else INK)
		_label(card,BUILD_HINTS[kind],12,MUTED)
		_build_costs[kind] = _label(card,_cost_text(definition.get("cost",{})),14,INK)
...
func _populate_army() -> void:
	_drawer_title.text = "Uma companhia. Um objetivo."
	_drawer_subtitle.text = "Escolha a ordem e toque no mapa. Os soldados coordenam a execução."
	_army_summary = _label(_drawer_content,"",20,WINE)
	_army_status = _label(_drawer_content,"",16,INK,true)
	var orders := _hbox(_drawer_content,8)
	for order in ["attack","defend","regroup","retreat"]:
		var btn := _button(orders,ORDER_NAMES[order],_choose_army.bind(order),110)
		btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		if order == "attack":
			_accent(btn,WINE)
	_label(_drawer_content,"Reforços · requer quartel e morador disponível",16,MUTED)
	var recruits := _hbox(_drawer_content,10)
	for role in ["lancer","archer"]:
		var row := _hbox(recruits,6)
		row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		_glyph(row,role,37)
		var button := _button(row,"Recrutar "+("lanceiro" if role == "lancer" else "arqueiro"),_recruit.bind(role))
		button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		button.tooltip_text = "6 alimentos + 4 madeiras + 1 morador disponível"
	_label(_drawer_content,"Cada reforço: 6 alimentos + 4 madeiras + 1 morador. Limite: 24 aliados.",14,MUTED,true)

func _recruit(role: String) -> void:
	command_requested.emit("recruit",{"role":role})
	refresh()
...
func _choose_army(order: String) -> void:
	close_panels()
	_mode_type = "army"
	set_mode(ORDER_NAMES.get(order,order)+" · toque no destino da companhia")
	army_selected.emit(order)
...
func _make_help() -> void:
	...
	for entry in [
		...
		["Atravesse o rio","Em Exército, escolha Atacar e toque no acampamento além da ponte. O botão Objetivo mostra o local. Você também pode defender, reagrupar e recuar a companhia."],
		...
		["Guarde sua partida","Menu → Salvar preserva a vila e o exército. Use Carregar para retomar. Reiniciar pede confirmação antes de começar de novo."]
	]:
...
func _cancel_mode() -> void:
	var previous_mode := _mode_type
	set_mode("")
	if previous_mode == "army":
		army_selected.emit("")
	else:
		build_selected.emit("")
...
func _refresh_speed() -> void:
	for value in _speed_buttons:
		...
		button.add_theme_stylebox_override("normal",_style(INK if value == _speed else Color("e8dec4"),Color("c6b998"),8,8))
...
func _focus_target() -> void:
	focus_requested.emit(Vector2i(30,14))
	show_message("Acampamento além do rio · objetivo da companhia")
...
func refresh() -> void:
	...
	_outcome.text = "Sua vila prosperou!" if bool(_sim.get("won")) else ("A vila precisa recomeçar" if bool(_sim.get("lost")) else "Faça sua vila prosperar")
...
func _refresh_army() -> void:
	var battle: Variant = _sim.get("battle")
	if battle == null:
		_army_summary.text = "A companhia está se preparando"
		_army_status.text = ""
		return
	var lancers := 0
	var archers := 0
	for unit in _array(battle.get("units")):
		if str(unit.get("team","")) == "ally" and int(unit.get("hp",0)) > 0:
			if str(unit.get("role","")) == "lancer":
				lancers += 1
			else:
				archers += 1
	_army_summary.text = "%d soldados · %d lanceiros · %d arqueiros" % [lancers+archers,lancers,archers]
	_army_status.text = str(battle.get("status"))
...
func _layout() -> void:
	...
	_brand.add_theme_font_size_override("font_size",17 if compact else 21)
	...
	_tabs["build"].custom_minimum_size.x = 115.0 if compact else 148.0
	_tabs["training"].custom_minimum_size.x = 91.0 if compact else 122.0
	_tabs["army"].custom_minimum_size.x = 94.0 if compact else 122.0
	_village_focus.custom_minimum_size.x = 57.0 if compact else 70.0
	_target_focus.custom_minimum_size.x = 64.0 if compact else 90.0
	_target_focus.text = "Alvo" if compact else "Objetivo"
	...
	_objectives_panel.size = Vector2(278 if compact else 302,0)
	var drawer_height := minf(302.0,maxf(150.0,height-top_height-margin*3-79.0))
	...
	if is_instance_valid(_build_grid):
		_build_grid.columns = 3 if compact else 5

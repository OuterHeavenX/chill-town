...
func _make_tutorial() -> void:
	...
	var intro := _label(box,"Você começa com o Prédio principal e a Escola de instrutores. Em Estradas, ligue as duas entradas: 1 pedra por trecho. Os construtores cuidam da obra.",15,INK,true)
	intro.max_lines_visible = 7
	_label(box,"Depois, construa lenhador, pedreira e horta. Forme os profissionais em Ofícios.",14,MUTED,true)
...
func _make_dock() -> void:
	...
	_tabs["road"].tooltip_text = "Planejar estradas · R · 1 pedra por trecho"
...
func _populate_build() -> void:
	...
	for kind in BUILD_ORDER:
		...
		button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		button.custom_minimum_size.y = 148
...
func _make_inspector() -> void:
	...
	_inspection_connection = _label(box,"",15,SUCCESS,true)
	...
	_inspection_details.max_lines_visible = 8
...
func _focus_inspected() -> void:
	...
	if not building.is_empty():
		focus_requested.emit(building.get("entrance",Vector2i(7,12)))
		entrance_highlighted.emit(building.get("entrance",Vector2i(7,12)))
...
func _make_help() -> void:
	...
	content.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	for entry in [
		["1. Ligue as entradas","A vila começa apenas com o Prédio principal e a Escola de instrutores. Em Estradas (R), arraste ou clique do marcador do principal até o da escola. Cada trecho custa 1 pedra; os construtores avançam sozinhos."],
		...
		["3. Forme a equipe","A escola precisa de conexão com o principal para funcionar. Em Ofícios, forme lenhador, canteiro e horticultor. Construtores erguem os prédios; serventes levam os materiais. Você nunca precisa direcionar pessoas."],
...
func _focus_village() -> void:
	focus_requested.emit(Vector2i(7,12))

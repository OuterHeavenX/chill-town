...
func run() -> void:
	...
	results.notes.append("Ablations affect rendering only: the 25 civilians still simulate and animate even when hidden.")
	print("BENCHMARK_BASELINE ", JSON.stringify(results.fixture))
	RenderingServer.viewport_set_measure_render_time(root.get_viewport_rid(), true)
	for case in [
		{"name":"baseline","size":43.0,"focus":Vector2i(13,13),"speed":1},
		{"name":"shadows_off","size":43.0,"focus":Vector2i(13,13),"speed":1},
		{"name":"grass_off","size":43.0,"focus":Vector2i(13,13),"speed":1},
		{"name":"trees_off","size":43.0,"focus":Vector2i(13,13),"speed":1},
		{"name":"people_off","size":43.0,"focus":Vector2i(13,13),"speed":1},
		{"name":"msaa_off","size":43.0,"focus":Vector2i(13,13),"speed":1},
		{"name":"tree_shadows_off","size":43.0,"focus":Vector2i(13,13),"speed":1},
		{"name":"shadow_distance_95","size":43.0,"focus":Vector2i(13,13),"speed":1}]:
...
func apply_ablation(kind: String) -> void:
	var forest: Node3D = game.world.terrain.get_node("OakAndPineForest")
	...
	match kind:
		...
		"grass_off":
			game.world.terrain.details.hide()
		"trees_off":
			forest.hide()
		"people_off":
			for person in game.world.people.values():
				person.hide()
		"msaa_off":
			root.msaa_3d = Viewport.MSAA_DISABLED
		"tree_shadows_off":
			set_casts_shadow(forest, false)
		"shadow_distance_95":
			sun.directional_shadow_max_distance = 95.0
		"baseline":
			results.geometry = {"forest":geometry_inventory(forest),"grass":geometry_inventory(game.world.terrain.details),"scene":geometry_inventory(game.world)}
			results.geometry.forest.tree_roots = forest.get_child_count()
			print("BENCHMARK_GEOMETRY ", JSON.stringify(results.geometry))

func set_casts_shadow(node: Node, enabled: bool) -> void:
	...
	for child in node.get_children():
		set_casts_shadow(child, enabled)

func geometry_inventory(node: Node) -> Dictionary:
	var totals := {"mesh_instances":0,"mesh_surfaces":0,"rendered_vertex_or_index_entries":0,"shadow_casting_instances":0}
	var mesh: Mesh
	var instances := 1
	...
	if mesh != null:
		totals.mesh_instances = 1
		totals.mesh_surfaces = mesh.get_surface_count()
		if node.cast_shadow != GeometryInstance3D.SHADOW_CASTING_SETTING_OFF:
			totals.shadow_casting_instances = 1
		for surface in range(mesh.get_surface_count()):
			...
			totals.rendered_vertex_or_index_entries += maxi(vertices,indices)*instances
	for child in node.get_children():
		var counts := geometry_inventory(child)
		for key in totals:

...
const Game = preload("res://presentation/frontier_game.gd")
var game: Node
var checks := 0
var failures: Array[String] = []
const SAVE_FILE := "user://vale-frontier-integration-save.json"
...
func begin_road(cell: Vector2i = Vector2i(8,12)) -> void:
...
func run() -> void:
 ...
 for cell in [Vector2i(7,12),Vector2i(12,12),Vector2i(10,16)]:
  expect(game.world.screen_to_cell(point(cell)) == cell,"3D picking matches cell projection "+str(cell))
 expect(not game.hud.blocks_pointer(point(Vector2i(8,12))),"road test starts on exposed terrain")
 begin_road(Vector2i(7,12))
 motion(point(Vector2i(12,12)))
 ...
 mouse(MOUSE_BUTTON_LEFT,false,point(Vector2i(12,12)))
 expect(game.sim.roads.size() == 5 and game.sim.reserved.stone == 5,"drag commits typed road cells and charges each new segment once")
 ...
 reset()
 begin_road()
 mouse(MOUSE_BUTTON_LEFT,false,point(Vector2i(8,12)))
 expect(game.sim.roads.size() == 1,"isolated click places one road tile")
 game._world_click(point(Vector2i(9,12)))
 ...
 motion(point(Vector2i(10,12)))
 mouse(MOUSE_BUTTON_LEFT,false,point(Vector2i(10,12)))
 ...
 key(KEY_E)
 mouse(MOUSE_BUTTON_LEFT,false,point(Vector2i(11,12)))
 ...
 mouse(MOUSE_BUTTON_WHEEL_UP,true,point(Vector2i(9,12)))
 mouse(MOUSE_BUTTON_LEFT,false,point(Vector2i(10,12)))
 expect(game.world.target_size < zoom_before and game.sim.roads.is_empty(),"zoom during drag cancels the pending stroke")
 camera_before = game.world.target_focus
 mouse(MOUSE_BUTTON_MIDDLE,true,point(Vector2i(9,12)))
 motion(point(Vector2i(10,12)),Vector2(45,20))
 mouse(MOUSE_BUTTON_MIDDLE,false,point(Vector2i(10,12)))
 ...
 motion(point(Vector2i(9,12)),Vector2(30,10))
 mouse(MOUSE_BUTTON_MIDDLE,false,point(Vector2i(9,12)))
 ...
 begin_road()
 reset()
 mouse(MOUSE_BUTTON_LEFT,false,point(Vector2i(9,12)))
 ...
 expect(game._write_save(SAVE_FILE),"frontier controller writes its isolated save format")
 begin_road()
 mouse(MOUSE_BUTTON_LEFT,false,point(Vector2i(8,12)))
 begin_road(Vector2i(9,12))
 expect(game._load_paths([SAVE_FILE]),"controller loads the prior frontier state")
 mouse(MOUSE_BUTTON_LEFT,false,point(Vector2i(11,12)))
 ...
 game.speed = 4
 game._process(0.01)
 expect(game.world.visual_speed == 4.0,"controller forwards selected speed to 3D animation")
 print("FRONTIER_INTEGRATION_RESULT checks=",checks," failures=",failures)
 game.queue_free()
 await frames(5)

...
func run() -> void:
	...
	var views: Array[String]=["Frente","Perfil","Costas"]
	for row in range(3):
		for column in range(3):
			...
			var environment:=WorldEnvironment.new();var env:=Environment.new();env.background_mode=Environment.BG_COLOR;env.background_color=Color("e5dfcf");env.ambient_light_source=Environment.AMBIENT_SOURCE_COLOR;env.ambient_light_color=Color("d6dce1");env.ambient_light_energy=0.45;env.tonemap_mode=Environment.TONE_MAPPER_ACES;environment.environment=env;studio.add_child(environment)
			var light:=DirectionalLight3D.new();light.rotation_degrees=Vector3(-35,-32,0);light.light_color=Color("fff3df");light.light_energy=1.0;light.shadow_enabled=true;studio.add_child(light)
			var fill:=DirectionalLight3D.new();fill.rotation_degrees=Vector3(-18,145,0);fill.light_color=Color("dce8f1");fill.light_energy=0.28;studio.add_child(fill)
			var person:=People.create(roles[row],1);person.rotation.y=[0.0,PI*0.5,PI][column];studio.add_child(person)
	...
	var path:="user://approved-people-turnarounds.png"

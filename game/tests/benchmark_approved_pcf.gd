...
const SHADOW_PROFILES = preload("res://tests/pcf_profiles_approved.gd")
...
func run() -> void:
	...
	warmup_seconds = float(argument_value("--warmup", "3"))
	sample_seconds = float(argument_value("--seconds", "5"))
	...
	for case in [
		{"name":"pcss-strategy","profile":"pcss","size":43.0,"focus":Vector2i(13,13),"speed":1},
		{"name":"pcss-default","profile":"pcss","size":30.0,"focus":Vector2i(11,13),"speed":1},
		{"name":"pcf-strategy","profile":"pcf","size":43.0,"focus":Vector2i(13,13),"speed":1},
		{"name":"pcf-default","profile":"pcf","size":30.0,"focus":Vector2i(11,13),"speed":1}]:
		await measure_case(case, baseline)
	for profile in ["pcss", "pcf"]:
		await record_pan(profile, baseline)
...
func measure_case(config: Dictionary, baseline: Dictionary) -> void:
	...
	var shadow_configuration := SHADOW_PROFILES.apply(game.world, config.profile)
	if game.world.people.size() != 25 or game.world.buildings.size() != 10:
		fail("Scene did not create all25 civilians and10 buildings; reject partial initialization")
		return
...
func record_pan(profile: String, baseline: Dictionary) -> void:
	game.set_process(false)
	assert(sim.restore(baseline.duplicate(true)))
	game.clock = CLOCK.new(100000)
	game.speed = 1
	game._clear_world()
	game.hud._toast.hide()
	game.world.focus_cell(Vector2i(11,13))
	game.world.target_size = 30.0
	SHADOW_PROFILES.apply(game.world, profile)
	if game.world.people.size() != 25:
		fail("Incomplete world before pan")
		return
	game.set_process(true)
	var warmup := Time.get_ticks_usec()
	while (Time.get_ticks_usec()-warmup)/1000000.0 < 3.0:
		await process_frame
	var from_focus: Vector3 = game.world.target_focus
	...
	var elapsed := 0.0
	print("PAN_CAPTURE_START ", profile)
	while elapsed < 5.0:
		await process_frame
		elapsed = (Time.get_ticks_usec()-start)/1000000.0
		game.world.target_focus = from_focus + Vector3(5.0*minf(elapsed/5.0,1.0),0,0)
		if elapsed >= next_capture and elapsed < 5.0:
			images.append(root.get_texture().get_image())
			times.append(elapsed)
			var c: Vector3 = game.world.camera.position
			camera_positions.append([c.x,c.y,c.z])
			next_capture += 1.0/15.0
	game.set_process(false)
	var folder := output_dir + "/pan-" + profile
	DirAccess.make_dir_recursive_absolute(folder)
	for index in range(images.size()):
		assert(images[index].save_png(folder + "/%04d.png" % index) == OK)
	write_json(folder + "/capture.json", {"profile":profile,"duration_seconds":elapsed,
		"sampling_target_fps":15,"frame_times_seconds":times,"camera_positions":camera_positions,
		"camera_size":30,"pan_displacement_world":[5,0,0],"world_people":game.world.people.size(),
		"note":"Readback during pan affects cadence; this is visual QA, not performance sampling."})
	images.clear()
	print("PAN_CAPTURE_COMPLETE ", profile, " frames=", times.size())

...
func _require(condition: bool, message: String = "Release verification failed") -> void:
	if not condition:
		push_error(message)
		quit(1)
...
func write_json(path: String, value: Variant) -> void:
	...
	_require(file != null, "Cannot write benchmark result " + path)
...
func run() -> void:
	...
	_require(warmup_seconds >= 0.0 and sample_seconds > 0.0)
	...
	# Identical legal fixture from the earlier isolated A/B comparison.
	var fixture_path := argument_value("--fixture", "user://approved-performance-final-validation/reference-fixture.json")
	var baseline: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(fixture_path))
	_require(sim.restore(baseline.duplicate(true)), "Cannot restore exact comparison fixture")
	_require(sim.tick == 7325 and sim.won and sim.workers.size() == 25, "Expected original completed legal fixture")
	_require(sim.buildings.size() == 10 and sim.roads.size() == 72, "Fixture buildings and roads changed")
	_require(sim.conservation_errors().is_empty(), "Original fixture conservation failed")
	...
	results = {"label":run_label, "generated_utc":Time.get_datetime_string_from_system(true),
		...
		"fixture_sha256":FileAccess.get_sha256(fixture_path), "fixture_source":fixture_path,
		...
		"samples":[], "notes":["Native development engine rendering the distributed game PCK; not the exported executable.",
			"Exact tick-7325 fixture restored from the earlier isolated comparison; no manual stepping during setup or sampling.",
	...
	results.quality = quality_state()
	print("BENCHMARK_BASELINE ", JSON.stringify(results.fixture))
	RenderingServer.viewport_set_measure_render_time(root.get_viewport_rid(), true)
	for case in [
		{"name":"default-1x","size":30.0,"focus":Vector2i(13,13),"speed":1},
...
func measure_case(config: Dictionary, baseline: Dictionary) -> void:
	game.set_process(false)
	_require(sim.restore(baseline.duplicate(true)), "Cannot restore legal baseline")
	...
	# Explicit focus before warmup; discard any sample that still loses it.
	root.grab_focus()
	print("BENCHMARK_WAITING_FOR_FOCUS ",config.name)
	var focus_deadline := Time.get_ticks_msec()+15000
	while not root.has_focus() and Time.get_ticks_msec()<focus_deadline:
		await process_frame
	if not root.has_focus():
		push_error("Benchmark focus timeout; no valid sample collected")
		quit(2)
		return
	...
	var sampling_start_utc := Time.get_datetime_string_from_system(true)
	...
	var sample := {"case":config.name,"camera_size":game.world.view_size,
		...
		"sampling_start_utc":sampling_start_utc, "sampling_end_utc":Time.get_datetime_string_from_system(true),
	...
	_require(screenshot.save_png(image_path) == OK)
	sample.screenshot = image_path
	_require(sample.paused_frames == 0 and sample.unfocused_frames == 0 and sample.tick_end > sample.tick_start, "Game must run throughout measurement")
	_require(sample.conservation_errors.is_empty(), "Conservation failed during benchmark")

func quality_state() -> Dictionary:
	var quality := {"msaa_3d":root.msaa_3d,"screen_space_aa":root.screen_space_aa,
		"shadow_atlas":ProjectSettings.get_setting("rendering/lights_and_shadows/directional_shadow/size"),
		"camera_far":game.world.camera.far,
		"civil_pace":game.get_script().get_script_constant_map().get("CIVIL_PACE")}
	for child in game.world.get_children():
		if child is DirectionalLight3D:
			quality.sun = {"shadows":child.shadow_enabled,"shadow_mode":child.directional_shadow_mode,
				"angular_distance":child.light_angular_distance,"blur":child.shadow_blur,"energy":child.light_energy}
		if child is WorldEnvironment:
			quality.ssao = child.environment.ssao_enabled
			quality.ambient_energy = child.environment.ambient_light_energy
	return quality
...
func build_mission()->void:
	for plan in [["house",Vector2i(4,6)],["house",Vector2i(8,5)],["lumber",Vector2i(3,18)],["quarry",Vector2i(12,19)],["farm",Vector2i(16,19)],["vineyard",Vector2i(17,5)],["winery",Vector2i(18,17)],["store",Vector2i(7,18)]]:
		...
		_require(result.ok,"legal approved mission construction "+plan[0]+": "+result.message)
	...
	_require(sim.command("road",{"cells":roads}).ok,"full mission network routes around the expanded main building and school")
	for role in ["lumberjack","stonecutter","farmer","vintner","vintner","servant","servant"]:
		_require(sim.command("train",{"role":role}).ok,"professional training request "+role)
